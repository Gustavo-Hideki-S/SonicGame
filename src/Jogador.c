...
void atualizarJogador( Jogador *j, GameWorld *gw, float delta ) {
    ...
    resolverColisaoJogadorItensMapa( j, gw );
    resolverColisaoJogadorInimigosMapa( j, gw );
...
static void resolverColisaoJogadorItensMapa( Jogador *j, GameWorld *gw ) {
    ...
    while ( el != NULL ) {
        ...
        // Colisão com Monitor de Vida
        if ( item->tipo == TIPO_ITEM_MONITOR_VIDA ) {

            ItemMonitorVida *monitorVida = (ItemMonitorVida*) item->objeto;

            if ( !monitorVida->ativo || monitorVida->estado == ESTADO_MONITOR_VIDA_COLETADO ) {
                el = el->proximo;
                continue;
            }

            QuadroAnimacao *qaItem = getQuadroAnimacaoAtualMonitorVida( monitorVida );
            
            Rectangle retColItemCalculado = {
                monitorVida->ret.x + qaItem->retColisao.x,
                monitorVida->ret.y + qaItem->retColisao.y,
                qaItem->retColisao.width,
                qaItem->retColisao.height
            };
        ...
        // Colisão com Monitor de Invencibilidade
        if ( item->tipo == TIPO_ITEM_MONITOR_INVENCIBILIDADE ) {

            ItemMonitorInvencivel *monitorInvencivel = (ItemMonitorInvencivel*) item->objeto;

            if ( !monitorInvencivel->ativo || monitorInvencivel->estado == ESTADO_MONITOR_INVENCIVEL_COLETADO ) {
                el = el->proximo;
                continue;
            }

            QuadroAnimacao *qaItem = getQuadroAnimacaoAtualMonitorInvencivel( monitorInvencivel );
            
            Rectangle retColItemCalculado = {
                monitorInvencivel->ret.x + qaItem->retColisao.x,
                monitorInvencivel->ret.y + qaItem->retColisao.y,
                qaItem->retColisao.width,
                qaItem->retColisao.height
            };
...
static void resolverColisaoJogadorInimigosMapa( Jogador *j, GameWorld *gw ) {
    ...
    while ( el != NULL ) {
        ...
        if ( inimigo->tipo == TIPO_INIMIGO_CRABMEAT ) {
            ...
            for ( int i = 0; i < 4; i++ ) {
                TiroCrabmeat *tiro = &crabmeat->tiros[i];

                if ( tiro->ativo && CheckCollisionRecs( retColCalculado, tiro->ret ) ) {
                    tiro->ativo = false;
        ...
        if ( inimigo->tipo == TIPO_INIMIGO_BUZZBOMBER ) {

            InimigoBuzzbomber *buzzbomber = (InimigoBuzzbomber*) inimigo->objeto;

            if ( !buzzbomber->ativo || buzzbomber->estado == ESTADO_INIMIGO_BUZZBOMBER_MORRENDO ) {
                el = el->proximo;
                continue;
            }

            qaInimigo = getQuadroAnimacaoAtualInimigoBuzzbomber( buzzbomber );
            ret = &buzzbomber->ret;

            Rectangle retColInimigoCalculado = {
                ret->x + qaInimigo->retColisao.x,
                ret->y + qaInimigo->retColisao.y,
                qaInimigo->retColisao.width,
                qaInimigo->retColisao.height
            };
            ...
            if ( CheckCollisionRecs( retColCalculado, retColInimigoCalculado ) ) {
                ...
                if ( j->estado >= ESTADO_JOGADOR_PULANDO && j->estado <= ESTADO_JOGADOR_PULANDO_CORRENDO ) {
                    j->vel.y = j->velPulo;
                    buzzbomber->estado = ESTADO_INIMIGO_BUZZBOMBER_MORRENDO;
                    gw->pontuacao += 100;
                    PlaySound( rm.somHitInimigo );
                } else if ( !j->invulneravel ) {
                    if ( j->quantidadeAneis > 0 ) {
            ...
            for ( int i = 0; i < 4; i++ ) {
                TiroBuzzbomber *tiro = &buzzbomber->tiros[i];
